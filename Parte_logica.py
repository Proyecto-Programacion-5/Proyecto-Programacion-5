...
class Juego:
    ...
    def Colocarminas(self):
        ...
        rng = random.Random()
        a = 0
        while a < len(self.minas_d) // 5:
            Mina = rng.randrange(0, len(self.cuadricula))
            if self.minas_d[self.cuadricula[Mina]] == False:
                self.minas_d[self.cuadricula[Mina]] = True
                a += 1
        self.banderas_restantes = len(self.minas_d) // 5
        self.banderas_max = self.banderas_restantes
