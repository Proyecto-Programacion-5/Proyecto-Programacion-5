...
class Juego:
    ...
    def __init__(self):
        ...
        self.ventana = pygame.display.set_mode((1500, 1000))
    ...
    def HacerCuadricula(self):
        x = (1500 - (self.width * self.size)) / 2
        y = (1000 - (self.width * self.size)) / 2
...
#            self.ContarCuadros()
    ...
    def PonerOQuitarBandera(self):
        for i in self.coord:
            if pygame.mouse.get_pos()[0] >= i[0] and pygame.mouse.get_pos()[0] <= i[0] + self.width and pygame.mouse.get_pos()[1] >= i[1] and pygame.mouse.get_pos()[1] <= i[1] + self.width:
